<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Administre y cierre la sesión de sus sesiones activas en otros navegadores y/o dispositivos.') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si es necesario, puede cerrar la sesión de todas las demás sesiones abiertas dentro de su navegador y en todos sus dispositivos. Algunas de sus sesiones recientes se enumeran a continuación; sin embargo, esta lista puede no ser exhaustiva. Si cree que su cuenta se ha visto comprometida, también debe actualizar su contraseña.') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('Sesión Actual') }}</span>
                                    @else
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Por favor, ingrese su contraseña para confirmar que desea cerrar la sesión de sus otras sesiones activas dentro de su navegador en todos sus dispositivos.') }}
